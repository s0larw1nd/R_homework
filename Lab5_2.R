...
#Лаб 5.2
data$Region <- as.factor(groups)
#Наивный Байес
install.packages("klaR")
library(klaR)

naive_data <- NaiveBayes(data$Region ~ ., data=data)
naive_data

pred <- predict(naive_data, data[, -8])$class
Acc <- mean(pred == data$Region)
...
myFormula <- Region ~ Birth_Rate + Death_Rate + Child_Mortality + Life_male + Life_female + Salary
data_ctree <- ctree(myFormula, data=trainData)
table(predict(data_ctree), trainData$Region)
plot(data_ctree)

test_predicted <- predict(data_ctree, newdata=testData)
table(test_predicted, testData$Region)
Acc <- mean(test_predicted == testData$Region)
paste("Точность=", round(100*Acc, 2), "%", sep = "")
...
rf <- randomForest(Region ~ .,data=trainData, ntree=100, proximity=TRUE)

table(predict(rf), trainData$Region)
test_predicted <- predict(rf, newdata=testData)
Acc <- mean(test_predicted == testData$Region)
paste("Точность=", round(100*Acc, 2), "%", sep = "")
